...
def scaffold_split_abs_value(dataset, smiles_list, task_idx=None, null_value=0, number_train=100, frac_valid=0.1,
                             frac_test=0.1, return_smiles=False):
    """
    Adapted from https://github.com/deepchem/deepchem/blob/master/deepchem/splits/splitters.py
    Split dataset by Bemis-Murcko scaffolds
    This function can also ignore examples containing null values for a
    ...
    # get train, valid test indices
    # train_cutoff = frac_train * len(smiles_list)
    train_number = number_train
    train_cutoff = (1 - frac_valid - frac_test) * len(smiles_list)
    valid_cutoff = train_cutoff + frac_valid * len(smiles_list)
    # test_cutoff = number_train + (frac_valid + frac_test) * len(smiles_list)
    train_idx, valid_idx, test_idx = [], [], []
    train_cutoff_idx = []
    for scaffold_set in all_scaffold_sets:
        if len(train_cutoff_idx) + len(scaffold_set) > train_cutoff:
            if len(train_cutoff_idx) + len(valid_idx) + len(scaffold_set) > valid_cutoff:
                test_idx.extend(scaffold_set)
            else:
                valid_idx.extend(scaffold_set)
        else:
            if len(train_idx) + len(scaffold_set) > train_number:
                train_cutoff_idx.extend(scaffold_set)
            else:
                train_idx.extend(scaffold_set)
                train_cutoff_idx.extend(scaffold_set)
